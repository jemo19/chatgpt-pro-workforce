...
| Download cleanup target is unsafe or changed | Target is a directory, symlink, glob, traversal, out-of-root path, ambiguous owner, in use, or hash differs from approved manifest | Record the exact blocked/failed outcome, perform no quarantine move, and require a new exact manifest if still needed | No scope widening |
| Cleanup would remove unhanded-off evidence | Accepted export, raw lineage decision, artifact index, or handoff is missing | Keep the file and complete acceptance/handoff before a new cleanup decision | No quarantine or purge |
